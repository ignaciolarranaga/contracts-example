...
export default async function bestProfession(
  ...
  const paidJobs = (await documentClient.query({
      TableName: process.env.TABLE_NAME!,
      IndexName: 'GSI3',
      KeyConditionExpression: 'PK3 = :pk3 AND SK3 BETWEEN :start AND :end',
      ExpressionAttributeValues: {
        ':pk3': 'Payment',
        ':start': event.arguments.filter?.start ? event.arguments.filter.start : '0000-01-01T00:00:00.000Z',
        ':end': event.arguments.filter?.end ? event.arguments.filter.end : '9999-12-31T23:59:69.999Z',
      }
    }).promise()).Items as Job[];

  let max = 0, bestProfession = null;
  const totals: { [profession: string]: number } = {};
  for (const job of paidJobs) {
    totals[job.profession!] = (totals[job.profession!] ? totals[job.profession!] : 0) + job.price;
